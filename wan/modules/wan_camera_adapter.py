...
class CameraAdapter(nn.Module):
    def __init__(self, in_dim, out_dim, kernel_size, stride, downscale_factor=8, num_residual_blocks=1):
        ...
        # Convolution: reduce spatial dimensions by a factor
        #  of 2 (without overlap)
        self.project_in = nn.Conv2d(in_dim * downscale_factor * downscale_factor, out_dim, kernel_size=kernel_size, stride=stride, padding=0)
        ...
        self.project_out = zero_module(nn.Conv2d(out_dim, out_dim, kernel_size=1, stride=1, padding=0))
    ...
    def forward(self, x):
        ...
        # Convolution operation
        x_conv = self.project_in(x_unshuffled)
