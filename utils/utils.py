...
def get_image_to_video_latent(validation_image_start, validation_image_end, video_length, sample_size):
    if validation_image_start is not None and validation_image_end is not None:
        ...
        if type(validation_image_start) is str and os.path.isfile(validation_image_start):
            ...
            image_start = image_start.resize([sample_size[1], sample_size[0]])
            clip_image = clip_image.resize([sample_size[1], sample_size[0]])
        elif type(validation_image_start) is np.ndarray:
            image_start = validation_image_start
            clip_image = None
        else:
            image_start = clip_image = validation_image_start
            # print(f'image_start is not str or np.ndarray, type is {type(image_start)}, {image_start}')
            image_start = [_image_start.resize([sample_size[1], sample_size[0]]) for _image_start in image_start]
            clip_image = [_clip_image.resize([sample_size[1], sample_size[0]]) for _clip_image in clip_image]
        image_end = None
        ...
        if type(image_start) is list:
            ...
            input_video_mask = torch.zeros_like(input_video[:, :1])
            input_video_mask[:, :, len(image_start):] = 255
        elif type(image_start) is np.ndarray:
            # (1, 3, window_size, h, w) range(0,1)
            window_size = image_start.shape[2]
            start_video = torch.from_numpy(image_start)
            input_video = torch.tile(torch.from_numpy(image_start)[:, :, :1], [1, 1, video_length, 1, 1])
            input_video[:, :, :window_size] = start_video

            input_video_mask = torch.zeros_like(input_video[:, :1])
            input_video_mask[:, :, window_size:] = 255
...
def calculate_padding(total_frames, chunk_size, window_size, temporal_compression_ratio):
    """
    计算满足特定整除条件的 total_frames_padding。
    
    条件:
    1. (total_frames_padding - chunk_size) % (chunk_size - window_size) == 0
       即: 满足滑动窗口的步长切分。
    2. (total_frames_padding - 1) % temporal_compression_ratio == 0
       即: 满足时间压缩比率的对齐要求 (通常用于 VAE 编解码对齐)。
    
    Args:
        total_frames (int): 原始总帧数
        chunk_size (int): 切片大小
        window_size (int): 窗口重叠大小
        temporal_compression_ratio (int): 时间压缩比率 (如 4, 8 等)
        
    Returns:
        int: 满足条件的最小 total_frames_padding (>= total_frames)
    """
    
    # 步长 (stride)
    stride = chunk_size - window_size
    
    if stride <= 0:
        raise ValueError("chunk_size 必须大于 window_size，否则步长为负或零。")

    # --- 步骤 1: 首先满足条件 2 ---
    # 目标: (N - 1) % R == 0  =>  N % R == 1
    # 我们先找到大于等于 total_frames 的第一个满足 N % R == 1 的数
    
    remainder = total_frames % temporal_compression_ratio
    
    # 计算需要补多少帧才能让余数变为 1
    # 公式逻辑: (目标余数 - 当前余数) % 模数
    frames_to_add = (1 - remainder) % temporal_compression_ratio
    
    current_frames = total_frames + frames_to_add
    
    # --- 步骤 2: 在满足条件 2 的基础上，寻找满足条件 1 的值 ---
    # 我们每次增加 temporal_compression_ratio，这样能保证条件 2 始终成立
    # 只需要检查条件 1: (N - chunk_size) % stride == 0
    
    while (current_frames - chunk_size) % stride != 0:
        current_frames += temporal_compression_ratio
        
    return current_frames

def split_config(config):
    """
    将 config 拆分为公共部分和私有部分。
    私有部分会被整理成列表，每个元素对应一个数据集的配置。
    
    规则：
    - train_data_meta, train_data_dir, train_data_class, train_data_prob 等会被拆分
    - video_sample_size 是 [H, W] 格式，保留在公共配置中
    - 其他非列表或长度不匹配的配置保留在公共配置中
    """
    import omegaconf 
    import copy
    assert 'ann_path' in config
    common_config = {}
    private_list = []
    
    # 1. 识别需要拆分的键 (基于 train_data_meta 确定数据集数量)
    # 假设 train_data_meta 是一定存在的列表
    num_datasets = len(config['ann_path'])
    
    # 初始化私有配置列表，每个元素是一个空字典
    private_list = [{} for _ in range(num_datasets)]
    
    # 需要拆分的键（每个数据集一个值）
    keys_to_split = ['ann_path', 'data_root', 'train_data_class', 'train_data_prob']

    for k, v in config.items():
        # 如果是需要拆分的键，且是列表且长度匹配
        if k in keys_to_split and isinstance(v, (list, omegaconf.listconfig.ListConfig)) and len(v) == num_datasets:
            for i in range(num_datasets):
                private_list[i][k] = v[i] if v[i] != '' else None
        else:
            # 其他配置保留在公共配置中
            common_config[k] = copy.deepcopy(v) 
    return private_list, common_config